...
describe('Agent Validation', () => {
  ...
  describe('Dynamic Agent Loading', () => {
    ...
    it('should merge static and dynamic templates', async () => {
      ...
      expect(result.templates).toHaveProperty('custom-agent')
    })
      ...
      const fileContext: ProjectFileContext = {
        ...
        agentTemplates: {
          ...
          'spawner.ts': {
            ...
            instructionsPrompt: 'Test user prompt',
            stepPrompt: 'Test step prompt',
            spawnableAgents: ['codebuffai-git-committer'],
            outputMode: 'last_message',
            includeMessageHistory: true,
            inheritParentSystemPrompt: false,
            toolNames: ['end_turn', 'spawn_agents'],
  ...
  describe('Schema Validation', () => {
    ...
    describe('Complex Schema Scenarios', () => {
      it('should handle both inputSchema prompt and params together', async () => {
        ...
        expect(inputPromptSchema.safeParse('').success).toBe(false)

        expect(
          paramsSchema.safeParse({ mode: 'fast', iterations: 5 }).success,
        ).toBe(true)
        expect(paramsSchema.safeParse({ mode: 'invalid' }).success).toBe(false)
        expect(paramsSchema.safeParse({ iterations: 5 }).success).toBe(false)
      })
        ...
        const fileContext: ProjectFileContext = {
          ...
          agentTemplates: {
            'error-context.ts': {
              ...
              stepPrompt: 'Test step prompt',
              inputSchema: {
                prompt: 10 as unknown as Record<string, never>,
              },
              outputMode: 'last_message',
              includeMessageHistory: true,
              inheritParentSystemPrompt: false,
  ...
  describe('HandleSteps Parsing', () => {
    ...
    test('should validate that handleSteps is a generator function', async () => {
      const agentTemplates = {
        'test-agent.ts': {
          ...mockAgentTemplate,
          handleSteps: 'function () { return "not a generator" }',
        },
      }
